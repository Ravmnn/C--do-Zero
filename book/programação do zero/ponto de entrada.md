...
Você talvez já tenha percebido ao ler as várias frases desse livro: Não existe uma, duas ou três linguagens de programação, hoje em dia, há dúzias e dúzias de diversas linguagens com estilos e propósitos diferentes. Há, também, linguagens que não foram feitas para programar, mas sim para estruturar dados (JSON, XML, YAML), estililizar coisas (CSS), entre outros. Para citar algumas, com propósito de exemplificação:
...
- *Lua*, uma linguagem brasileira, criada no Rio de Janeiro com o propósito de ser fácil de integrar em outros softwares. Ela será melhor aborda mais a frente.
...
[^4]: Um framework é um conjunto de ferramentas, serviços e/ou bibliotecas com o propósito de auxiliar no desenvolvimento de algo. A diferença de um framework e uma biblioteca, é que uma biblioteca possui apenas um propósito (ou pelo menos, deveria), ou seja, fornece apenas uma funcionalidade específica. Já um framework pode abranger e fornecer muitos mais recursos e funcionalidades. Uma analogia simples seria comparar uma biblioteca a uma chave de fenda que você tenha tirado de uma caixa de ferramentas. Já o framework pode ser comparado a caixa de ferramentas inteira.