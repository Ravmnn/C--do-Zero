...
Logo no início, eu disse que o único idioma que um computador entende é o Código de Máquina. Levando isso em conta, como que um computador iria entender código escrito em uma linguagem de programação?. Antes de continuar a leitura, pare por alguns segundos e tente imaginar uma forma de fazer um computador, que só entende código de máquina, compreender uma linguagem como C++... Encare isso como seu primeiro trabalho como um programador![^1] Não tem problema se errar na resposta, já que ela pode não ser tão simples assim.


Na realidade, tanto Assembly quanto todas as linguagens de programação não existem!
Assembly é convertido em código de máquina por uma ferramenta chamada *Assembler*, então, de certa forma, Assembly em si não existe. Se sua resposta à pergunta anterior foi algo como *"converter o código original em código de máquina"*, então parabéns, seu cérebro precisa ser estudado. Se não tiver sido, não tem problema. Dúvido que, se eu fosse iniciante, iria conseguir acertar a resposta.
...
1. *Interpretação*: O código fonte passa por uma ferramenta chamada de interpretador, que executa o código linha por linha em tempo real. O código original é representado na memória utilizando um formato chamado de *Abstract Syntactic Tree (Árvore Sintática Abstrata; AST)* . É prático, a troco de desempenho, já que representar (e converter código em) uma AST tende a ser pesado.
